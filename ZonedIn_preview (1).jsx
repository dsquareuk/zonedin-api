...
// ── supabase ───────────────────────────────────────────────────────────────────
const SUPABASE_URL  = "https://takormabbagvdckpxpox.supabase.co";
const SUPABASE_KEY  = "sb_publishable_yOWhO9_U1TkyUmsvHy1CCQ_vDl9TTUi";
...
const LS = {
  ...
  clear: ()   => { try { Object.keys(localStorage).filter(k=>k.startsWith("zi_")).forEach(k=>localStorage.removeItem(k)); } catch {} },
};
...
const ONBOARDING = [
  { emoji:"🗺️", title:"Map Your Zones",  body:"Set three focused tasks each day. Gold is your must-do, Diamond builds momentum, Platinum is your breakthrough. Tap a zone card to activate it." },
...
function AuthScreen({ onAuth }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0d1117",display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:'"Outfit",ui-sans-serif'}}>
      <div style={{maxWidth:380,width:"100%",textAlign:"center"}}>
        <div style={{marginBottom:24,display:"flex",justifyContent:"center"}}><ZonedInMark size={56}/></div>
        <h1 style={{margin:"0 0 6px",fontSize:28,fontWeight:900,color:"#F9FAFB",letterSpacing:-0.5}}>ZonedIn</h1>
        <p style={{margin:"0 0 32px",fontSize:14,color:"rgba(249,250,251,0.5)"}}>Clarity Beats Chaos</p>

        {!sent ? (
          <div style={{background:"#161b22",borderRadius:20,padding:"32px 28px",border:"1px solid rgba(255,255,255,0.08)"}}>
            <h2 style={{margin:"0 0 6px",fontSize:18,fontWeight:800,color:"#F9FAFB"}}>Sign in to ZonedIn</h2>
            <p style={{margin:"0 0 20px",fontSize:13,color:"rgba(249,250,251,0.5)",lineHeight:1.6}}>Enter your email and we'll send you a magic link — no password needed.</p>
            <input
              type="email" value={email} onChange={e=>setEmail(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&handleLogin()}
              placeholder="your@email.com"
              style={{width:"100%",padding:"13px 16px",borderRadius:12,border:"1.5px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#F9FAFB",fontSize:14,fontFamily:'"Outfit",ui-sans-serif',outline:"none",boxSizing:"border-box",marginBottom:12}}
            />
            {error && <p style={{margin:"0 0 10px",fontSize:12,color:"#EF4444"}}>{error}</p>}
            <button onClick={handleLogin} disabled={loading||!email.trim()} style={{width:"100%",padding:"14px",borderRadius:50,border:"none",background:loading||!email.trim()?"#374151":"#00C2B2",color:"#fff",fontSize:15,fontWeight:800,cursor:loading||!email.trim()?"not-allowed":"pointer",fontFamily:'"Outfit",ui-sans-serif',transition:"all 0.2s"}}>
              {loading ? "Sending..." : "Send Magic Link ✦"}
            </button>
          </div>
        ) : (
          <div style={{background:"#161b22",borderRadius:20,padding:"32px 28px",border:"1px solid rgba(0,194,178,0.2)"}}>
            <div style={{fontSize:48,marginBottom:16}}>📬</div>
            <h2 style={{margin:"0 0 8px",fontSize:18,fontWeight:800,color:"#F9FAFB"}}>Check your inbox</h2>
            <p style={{margin:"0 0 16px",fontSize:13,color:"rgba(249,250,251,0.5)",lineHeight:1.7}}>We sent a magic link to <strong style={{color:"#00C2B2"}}>{email}</strong>. Click it to sign in — it expires in 1 hour.</p>
            <button onClick={()=>setSent(false)} style={{fontSize:13,background:"none",border:"none",color:"rgba(249,250,251,0.4)",cursor:"pointer",fontFamily:'"Outfit",ui-sans-serif'}}>Use a different email</button>
          </div>
        )}
      </div>
    </div>
...
function SessionLock({ secondsLeft, activeZone, onEnd, onCompleteTask, sessionDuration }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"linear-gradient(135deg,#0d1117 0%,#0a1a1a 60%,#001a18 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:28,fontFamily:'"Outfit",ui-sans-serif'}}>
      ...
      <div style={{display:"flex",gap:10,width:"100%",maxWidth:340,padding:"0 4px"}}>
        <button onClick={onCompleteTask}
          onPointerEnter={e=>{e.currentTarget.style.background="#00A89E";e.currentTarget.style.transform="scale(1.02)";}}
          onPointerLeave={e=>{e.currentTarget.style.background="#00C2B2";e.currentTarget.style.transform="scale(1)";}}
          style={{flex:1,padding:"15px 12px",borderRadius:50,border:"none",background:"#00C2B2",color:"#fff",fontSize:15,fontWeight:900,cursor:"pointer",fontFamily:'"Outfit",ui-sans-serif',boxShadow:"0 0 20px rgba(0,194,178,0.4)",transition:"all 0.2s",whiteSpace:"nowrap"}}>
          ✓ Task Complete
        </button>
        <button onClick={onEnd}
          onPointerEnter={e=>e.currentTarget.style.background="rgba(0,194,178,0.1)"}
          onPointerLeave={e=>e.currentTarget.style.background="transparent"}
          style={{flex:1,padding:"15px 12px",borderRadius:50,border:"1.5px solid rgba(0,194,178,0.3)",background:"transparent",color:"rgba(249,250,251,0.55)",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:'"Outfit",ui-sans-serif',transition:"all 0.2s",whiteSpace:"nowrap"}}>
          End Session
        </button>
      </div>
    </div>
...
function SessionComplete({ session, onDismiss, dark }) {
  ...
  const isTaskComplete = session.taskCompleted;
  useEffect(()=>{ if (isTaskComplete) playCelebrationSound(); }, [isTaskComplete]);
  return (
    <div style={{position:"fixed",inset:0,zIndex:9998,background:isTaskComplete?"rgba(0,0,0,0.75)":"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:'"Outfit",ui-sans-serif'}}>
      <div style={{background:bg,borderRadius:24,padding:"40px 32px",maxWidth:380,width:"100%",textAlign:"center",boxShadow:"0 24px 80px rgba(0,0,0,0.25)",position:"relative",overflow:"hidden"}}>
        {isTaskComplete&&<div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 50% 0%,rgba(0,194,178,0.15) 0%,transparent 70%)",pointerEvents:"none"}}/>}
        <div style={{fontSize:64,marginBottom:10,display:"inline-block",animation:isTaskComplete?"taskPop 0.5s cubic-bezier(0.34,1.56,0.64,1) both":"none"}}>
          {isTaskComplete?"🏆":session.completed?"🎯":"⏹️"}
        </div>
        <h2 style={{margin:"0 0 8px",fontSize:24,fontWeight:900,color:isTaskComplete?"#00C2B2":fg,letterSpacing:-0.5}}>
          {isTaskComplete?"Task Complete!":session.completed?"Zone Complete!":"Session Ended"}
        </h2>
        {isTaskComplete&&<div style={{display:"inline-block",background:"linear-gradient(135deg,rgba(0,194,178,0.15),rgba(255,214,107,0.15))",border:"1.5px solid rgba(0,194,178,0.3)",borderRadius:50,padding:"4px 16px",fontSize:12,fontWeight:800,color:"#00C2B2",marginBottom:12,letterSpacing:1,textTransform:"uppercase"}}>🎉 Congratulations</div>}
        <p style={{margin:"0 0 24px",fontSize:14,color:sub,lineHeight:1.7}}>
          {isTaskComplete?`You completed "${session.zone}" — that's a real win.`:session.completed?"You locked in and delivered. Brilliant.":"Good work — every minute in the zone counts."}
        </p>
        <div style={{display:"flex",gap:10,marginBottom:24}}>
          <div style={{flex:1,padding:"16px",borderRadius:14,background:isTaskComplete?"rgba(0,194,178,0.08)":cardBg,border:isTaskComplete?"1.5px solid rgba(0,194,178,0.2)":"none",textAlign:"center"}}>
        ...
        <button onClick={onDismiss} style={{width:"100%",padding:"14px",borderRadius:50,border:"none",background:isTaskComplete?"linear-gradient(135deg,#00C2B2,#00A89E)":"#00C2B2",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:'"Outfit",ui-sans-serif',boxShadow:isTaskComplete?"0 4px 20px rgba(0,194,178,0.4)":"none"}}>
          {isTaskComplete?"Let's Keep Going 🚀":"Back to ZonedIn →"}
        </button>
      </div>
      <style>{`@keyframes taskPop{0%{transform:scale(0.5) rotate(-10deg);opacity:0}60%{transform:scale(1.2) rotate(5deg)}100%{transform:scale(1) rotate(0deg);opacity:1}}`}</style>
    </div>
  );
}
...
export default function ZonedInMVP() {
  ...
  const [streakTier,     setStreakTier]    = useState("none");
  const [tierHistory,    setTierHistory]   = useState([]);
  const [totalPoints,    setTotalPoints]   = useState(0);
  const [todayPointsLogged, setTodayPointsLogged] = useState(null);
  const [goldZone,       setGoldZone]      = useState("");
  const [diamondZone,    setDiamondZone]   = useState("");
  const [platinumZone,   setPlatinumZone]  = useState("");
  const [zoneLastSet,    setZoneLastSet]   = useState(null);
  const [completedZones, setCompletedZones]= useState([]);
  const [activeZoneKey,  setActiveZoneKey] = useState("gold");
  ...
  const [hasSeenCompleteHint, setHasSeenCompleteHint]= useState(false);
  const [savingZones,         setSavingZones]        = useState(false);
  const intervalRef = useRef(null);
  const zoneSaveTimer = useRef(null);
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const localStreak = LS.get("zi_streak", null);
      ...
      if (profile) {
        ...
        setStreakTier(profile.streak_tier ?? "none");
        setTierHistory(profile.tier_history ?? []);
        setTotalPoints(profile.total_points ?? 0);
        setTodayPointsLogged(profile.today_points_logged ?? null);
      } else if (localStreak !== null) {
        // Migrate from localStorage
        await migrateFromLocal();
      }

      // Load today's zones
      const { data: zones } = await supabase.from("zones")
        .select("*").eq("user_id", user.id).eq("date", todayStr()).single();
      if (zones) {
        setGoldZone(zones.gold ?? "");
        setDiamondZone(zones.diamond ?? "");
        setPlatinumZone(zones.platinum ?? "");
        setCompletedZones(zones.completed_zones ?? []);
        setWentWell(zones.went_well ?? "");
        setEnergy(zones.energy ?? 3);
        setMood(zones.mood ?? null);
        setActiveZoneKey(zones.active_zone_key ?? "gold");
        setZoneLastSet(zones.zone_last_set ?? null);
      }

      // Load sessions (last 20)
      const { data: sessions } = await supabase.from("sessions")
        .select("*").eq("user_id", user.id)
        .order("created_at", { ascending: false }).limit(20);
      if (sessions) {
        setSessionHistory(sessions.map(s => ({
          date: s.date, zone: s.zone_label, emoji: s.zone_emoji,
          minutes: s.minutes, completed: s.completed, taskCompleted: s.task_completed,
        })));
      }

      // Load insights (last 7)
      const { data: insights } = await supabase.from("insights")
        .select("*").eq("user_id", user.id)
        .order("created_at", { ascending: false }).limit(7);
      if (insights) {
        setInsightHistory(insights.map(i => ({
          date: i.date, text: i.text, energy: i.energy, mood: i.mood,
        })));
      }
  ...
  const migrateFromLocal = async () => {
    const pts = await supabase.from("profiles").upsert({
      id: user.id,
      streak: LS.get("zi_streak", 0),
      last_streak_date: LS.get("zi_streak_date", null),
      streak_tier: LS.get("zi_streak_tier", "none"),
      tier_history: LS.get("zi_tier_history", []),
      total_points: LS.get("zi_points", 0),
      today_points_logged: LS.get("zi_points_date", null),
    });
    LS.clear();
    return pts;
  };
  ...
  const saveZonesToDB = useCallback(async (zoneData) => {
    if (!user) return;
    setSavingZones(true);
    await supabase.from("zones").upsert({
      user_id: user.id, date: todayStr(), ...zoneData,
    }, { onConflict: "user_id,date" });
    setSavingZones(false);
  }, [user]);

  // debounced zone save — fires 1.5s after last keystroke
  const debouncedSaveZones = useCallback((zoneData) => {
    clearTimeout(zoneSaveTimer.current);
    zoneSaveTimer.current = setTimeout(() => saveZonesToDB(zoneData), 1500);
  }, [saveZonesToDB]);

  const saveSessionToDB = useCallback(async (session) => {
    if (!user) return;
    await supabase.from("sessions").insert({
      user_id: user.id,
      zone_label: session.zone, zone_emoji: session.emoji,
      zone_key: session.zoneKey, minutes: session.minutes,
      completed: session.completed, task_completed: session.taskCompleted ?? false,
      date: session.date,
    });
  }, [user]);

  const saveInsightToDB = useCallback(async (insight) => {
    if (!user) return;
    await supabase.from("insights").insert({
      user_id: user.id, date: insight.date,
      text: insight.text, energy: insight.energy, mood: insight.mood,
    });
  }, [user]);

  // ── persist dark mode locally (preference only) ───────────────────────────────
  ...
  const handleDailyReset = async () => {
    ...
    await saveZonesToDB({
      gold:"", diamond:"", platinum:"", completed_zones:[],
      went_well:"", mood:null, energy:3, active_zone_key:"gold",
      zone_last_set: todayStr(),
    });
  };

  // ── zone config ───────────────────────────────────────────────────────────────
  const handleZoneChange = (setter, field) => (v) => {
    setter(v);
    const zls = zoneLastSet || todayStr();
    if (!zoneLastSet) setZoneLastSet(zls);
    debouncedSaveZones({
      gold: field==="gold"?v:goldZone,
      diamond: field==="diamond"?v:diamondZone,
      platinum: field==="platinum"?v:platinumZone,
      completed_zones: completedZones,
      went_well: wentWell, energy, mood,
      active_zone_key: activeZoneKey,
      zone_last_set: zls,
    });
  };

  const zoneConfig = [
    { key:"gold",     val:goldZone,     set:handleZoneChange(setGoldZone,"gold"),         label:"Gold Zone",     sublabel:"Non-negotiable", emoji:"🥇", color:"#F59E0B", bg:darkMode?"rgba(245,158,11,0.12)":"rgba(245,158,11,0.08)",   border:"rgba(245,158,11,0.35)",   placeholder:"e.g. Finish the client proposal"       },
    { key:"diamond",  val:diamondZone,  set:handleZoneChange(setDiamondZone,"diamond"),   label:"Diamond Zone",  sublabel:"Momentum",       emoji:"💎", color:"#60A5FA", bg:darkMode?"rgba(96,165,250,0.12)":"rgba(96,165,250,0.08)",   border:"rgba(96,165,250,0.35)",   placeholder:"e.g. Review and respond to key emails" },
    { key:"platinum", val:platinumZone, set:handleZoneChange(setPlatinumZone,"platinum"), label:"Platinum Zone", sublabel:"Breakthrough",   emoji:"🏆", color:"#A78BFA", bg:darkMode?"rgba(167,139,250,0.12)":"rgba(167,139,250,0.08)", border:"rgba(167,139,250,0.35)", placeholder:"e.g. Start the new product research doc" },
  ];
  ...
  const toggleZoneDone = async (e, k) => {
    e.stopPropagation();
    const next    = completedZones.includes(k) ? completedZones.filter(x=>x!==k) : [...completedZones, k];
    const newTier = getStreakTier(next);
    const t       = todayStr();
    const newHist = [{ date:t, tier:newTier }, ...tierHistory.filter(h=>h.date!==t)].slice(0,30);
    setCompletedZones(next);
    setStreakTier(newTier);
    setTierHistory(newHist);
    await saveZonesToDB({
      gold:goldZone, diamond:diamondZone, platinum:platinumZone,
      completed_zones:next, went_well:wentWell, energy, mood,
      active_zone_key:activeZoneKey, zone_last_set:zoneLastSet,
    });
    await saveProfile({ streak_tier:newTier, tier_history:newHist });
  };
  ...
  const endSession = useCallback(() => {
    ...
    const session = { date:todayShort(), zone:activeZoneLabel, emoji:activeZone?.emoji||"🥇", zoneKey:activeZoneKey, minutes:minutesDone, completed };
    ...
    if (lastStreakDate !== t) {
      const ns = streak + 1;
      setStreak(ns); setLastStreakDate(t);
      saveProfile({ streak:ns, last_streak_date:t });
    }
  }, [secondsLeft, sessionDuration, activeZoneLabel, activeZone, activeZoneKey, lastStreakDate, streak]);

  const completeTask = useCallback(() => {
    setSessionActive(false);
    clearInterval(intervalRef.current);
    const minutesDone = Math.round((sessionDuration * 60 - secondsLeft) / 60);
    const session = { date:todayShort(), zone:activeZoneLabel, emoji:activeZone?.emoji||"🥇", zoneKey:activeZoneKey, minutes:minutesDone, completed:true, taskCompleted:true };
    setSessionHistory(prev => [session, ...prev.slice(0,19)]);
    setLastSession(session);
    saveSessionToDB(session);
    // auto-mark zone done
    if (!completedZones.includes(activeZoneKey)) {
      const next    = [...completedZones, activeZoneKey];
      const newTier = getStreakTier(next);
      const t       = todayStr();
      const newHist = [{ date:t, tier:newTier }, ...tierHistory.filter(h=>h.date!==t)].slice(0,30);
      setCompletedZones(next); setStreakTier(newTier); setTierHistory(newHist);
      saveZonesToDB({ gold:goldZone, diamond:diamondZone, platinum:platinumZone, completed_zones:next, went_well:wentWell, energy, mood, active_zone_key:activeZoneKey, zone_last_set:zoneLastSet });
      saveProfile({ streak_tier:newTier, tier_history:newHist });
    }
    const t = todayStr();
    if (lastStreakDate !== t) {
      const ns = streak + 1;
  ...
  }, [secondsLeft, sessionDuration, activeZoneLabel, activeZone, activeZoneKey, lastStreakDate, streak, completedZones, tierHistory, goldZone, diamondZone, platinumZone, wentWell, energy, mood, zoneLastSet]);
  ...
  // ── points ────────────────────────────────────────────────────────────────────
  const logTodayPoints = async (hasInsight) => {
    const t = todayStr();
    if (todayPointsLogged === t) return 0;
    const todaySess = sessionHistory.filter(s => s.date === todayShort());
    const pts = calcDayPoints(
      completedZones, Math.max(0, todaySess.length-1),
      todaySess.some(s=>s.completed), !!wentWell.trim(),
      hasInsight, !!(mood && energy), streakTier
    );
    const newTotal = totalPoints + pts;
    setTotalPoints(newTotal);
    setTodayPointsLogged(t);
    await saveProfile({ total_points:newTotal, today_points_logged:t });
    return pts;
  };
  ...
  const generateInsight = async () => {
    ...
    const recentSessions = sessionHistory.slice(0,5).map(s=>`${s.date}: ${s.minutes} min on "${s.zone}" (${s.completed?"completed":"partial"})`).join("\n");
...
🔧 What to try: [suggestion]`;
    ...
    try {
      const res  = await fetch("https://zonedin-api.vercel.app/api/insight", {
      ...
      if (text) {
        const entry = { date:todayShort(), text:text.trim(), energy, mood };
        setInsightHistory(prev=>[entry,...prev.slice(0,6)]);
        await saveInsightToDB(entry);
        await logTodayPoints(true);
        // save reflection to zones table
        await saveZonesToDB({ gold:goldZone, diamond:diamondZone, platinum:platinumZone, completed_zones:completedZones, went_well:wentWell, energy, mood, active_zone_key:activeZoneKey, zone_last_set:zoneLastSet });
  ...
  if (authLoading) return (
    <div style={{minHeight:"100vh",background:"#0d1117",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:'"Outfit",ui-sans-serif'}}>
      <div style={{textAlign:"center"}}>
        <ZonedInMark size={48}/>
        <p style={{color:"rgba(249,250,251,0.4)",marginTop:16,fontSize:14}}>Loading...</p>
      </div>
    </div>
  );
  ...
  if (dbLoading) return (
    <div style={{minHeight:"100vh",background:brand.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:'"Outfit",ui-sans-serif'}}>
      <div style={{textAlign:"center"}}>
        <ZonedInMark size={48}/>
        <p style={{color:brand.subText,marginTop:16,fontSize:14}}>Loading your zones...</p>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {!hasOnboarded && <Onboarding dark={darkMode} onComplete={async()=>{ setHasOnboarded(true); await saveProfile({onboarded:true}); }}/>}
      {sessionActive && <SessionLock secondsLeft={secondsLeft} activeZone={activeZoneLabel} onEnd={endSession} onCompleteTask={completeTask} sessionDuration={sessionDuration}/>}
      ...
      <div style={{minHeight:"100vh",background:brand.bg,color:brand.charcoal,fontFamily:'"Outfit",ui-sans-serif',padding:"24px 16px"}}>
        <div style={{maxWidth:680,margin:"0 auto",display:"flex",flexDirection:"column",gap:20}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    ...
                    <button onClick={()=>supabase.auth.signOut()} title="Sign out" style={{background:brand.inputBg,border:`1px solid ${brand.border}`,borderRadius:50,width:34,height:34,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,color:brand.subText}}>
                      ↩
                    </button>
                  </div>
                </div>
                <p style={{margin:0,fontSize:13,color:brand.subText,fontWeight:500}}>
                  {user.email} · Zone in. Gain clarity. Level up.
                  {savingZones && <span style={{marginLeft:8,color:brand.teal,fontSize:11}}>saving...</span>}
                </p>
              </div>
            </div>
          </header>
          ...
          <section className="card-anim" style={{...cardStyle,animationDelay:"0.05s"}}>
            ...
            <p style={{margin:"4px 0 14px",fontSize:13,color:brand.subText}}>Set your three tasks for today. Tick the ○ checkbox when complete.</p>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {zoneConfig.map(({key,val,set,label,sublabel,emoji,color,bg,border:zb,placeholder})=>{
                ...
                return (
                  <div key={key} onClick={()=>!isDone&&setActiveZoneKey(key)} style={{borderRadius:14,border:`2px solid ${isDone?brand.border:isActive?color:brand.border}`,background:isDone?brand.inputBg:isActive?bg:brand.inputBg,padding:"12px 14px",cursor:isDone?"default":"pointer",transition:"all 0.2s",boxShadow:isActive&&!isDone?`0 0 0 3px ${zb}`:"none",opacity:isDone?0.6:1,position:"relative"}}>
                    {!hasSeenCompleteHint&&key==="gold"&&val.trim()&&!isDone&&(
                      <div onClick={e=>{e.stopPropagation();setHasSeenCompleteHint(true);}} style={{position:"absolute",left:6,top:-36,background:"#1B1F23",color:"#fff",fontSize:11,fontWeight:700,padding:"5px 10px",borderRadius:8,whiteSpace:"nowrap",zIndex:10,cursor:"pointer",boxShadow:"0 4px 12px rgba(0,0,0,0.3)"}}>
                        Tick here when done ✓
                        <div style={{position:"absolute",bottom:-6,left:14,width:0,height:0,borderLeft:"6px solid transparent",borderRight:"6px solid transparent",borderTop:"6px solid #1B1F23"}}/>
                      </div>
                    )}
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <div onClick={e=>e.stopPropagation()} style={{flexShrink:0}}>
                        {(isDone||val.trim()) ? (
                          <button onClick={e=>{e.stopPropagation();toggleZoneDone(e,key);setHasSeenCompleteHint(true);}}
                            style={{width:28,height:28,borderRadius:8,border:`2px solid ${isDone?brand.teal:brand.border}`,background:isDone?"rgba(0,194,178,0.12)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.15s",flexShrink:0,fontSize:15,color:isDone?brand.teal:brand.subText}}>
                            {isDone?"✓":"○"}
                          </button>
                        ) : (
                          <div style={{width:28,height:28,borderRadius:8,border:`2px dashed ${brand.border}`,background:"transparent",flexShrink:0}}/>
                        )}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                          ...
                          <span style={{fontSize:11,color:brand.subText}}>{sublabel}</span>
                          {isActive&&!isDone&&<span style={{fontSize:10,fontWeight:800,letterSpacing:1,textTransform:"uppercase",color,border:`1px solid ${color}`,padding:"2px 8px",borderRadius:50,marginLeft:"auto"}}>Active</span>}
                        </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"12px 14px",borderRadius:12,background:activeZone?.val?.trim()?activeZone.bg:brand.inputBg,border:`1.5px solid ${activeZone?.val?.trim()?activeZone.color:brand.border}`,transition:"all 0.25s"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontWeight:800,fontSize:14,color:activeZone?.val?.trim()?activeZone.color:brand.subText,fontStyle:activeZone?.val?.trim()?"normal":"italic"}}>
                  {activeZone?.val?.trim()||"No task set — add one in Zone Mapping above"}
                </div>
              </div>
                ...
                {(historyExpanded?sessionHistory:sessionHistory.slice(0,3)).map((s,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:12,background:brand.bg,border:`1px solid ${brand.border}`,fontSize:13}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                      ...
                      <span>{s.taskCompleted?"🏆":s.completed?"✅":"⏹️"}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="card-anim" style={{...cardStyle,animationDelay:"0.2s",overflow:"hidden"}}>
            ...
            {isStreakMilestone&&(
              <div style={{marginTop:16,padding:"16px",borderRadius:14,background:`linear-gradient(135deg,${currentTierCfg.bg},rgba(255,214,107,0.08))`,border:`1.5px solid ${currentTierCfg.border}`}}>
                ...
                <p style={{margin:"0 0 12px",fontSize:13,color:brand.subText,lineHeight:1.6}}>You've hit {streak} days. Time to analyse your performance.</p>
                {!streakReport ? (
